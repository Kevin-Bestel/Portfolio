...
function openAccordion(e) {
    e.stopPropagation();
    console.log(e)
    let clickedElement = e.srcElement.closest(".trigger");
...
var rm = function(){
    jQuery('.trigger .accordion-trigger').html('{...}');
    jQuery('#added').remove();
    jQuery('.trigger').children('.sub-accordion-container').toggleClass('active');
}
...
jQuery(document).ready(function(){
    jQuery(".trigger .accordion-trigger").click(function(e){
        //$(this).find("code").css("display", "inline-block");
        if (!jQuery(this).parent('.trigger').children('.sub-accordion-container').hasClass('active')){
            jQuery('#added').remove();
            jQuery(this).html('{');
            jQuery(this).parent('.trigger').append("<code class='accordion-trigger' id='added' onclick='rm();'>}</code>");
        }
        else {
            jQuery(this).html('{...}');
            jQuery('#added').remove();
        ...
        jQuery(this).parent('.trigger').children('.sub-accordion-container').toggleClass('active');
    });


//Unter-Container wird beim hovern über die Funktionsklammer aufgeklappt und geschlossen
    jQuery(".sub-trigger .sub-accordion-trigger").hover(function(e){

            if (!jQuery(this).parent('.sub-trigger').children('.text-container').hasClass('active')){
                jQuery('#added').remove();
                jQuery(this).html('{');
                jQuery(this).parent('.sub-trigger').append("<code class='sub-accordion-trigger' id='added' hover='rm();'>}</code>");
            }
            else {
                jQuery(this).html('{...}');
                jQuery('#added').remove();
            ...
            jQuery(this).parent('.sub-trigger').children('.text-container').toggleClass('active');
        });
